...
int background_ncdm_distribution(
  ...
  /** - b) deal now with case of reading analytical function */
    ...
    /**
       ...
       species.
    */
    if(pba->background_ncdm_psd[n_ncdm] == _fermi_dirac_){
    /**************************************************/
   ...
   /* We must use the list of extra parameters read in input, stored in the
    ...
    /* Assign good names to entries in param */
    double alpha = param[0]; // GFA
    double y_avg = param[1];
    /* double x_norm= param[2]; */

    /* We are always using this distribution as WDM so normalisation
    not important (i.e. we impose both Omega_ncdm and m_ncdm
    for this species).
    Note: q=y if we take T_ncdm = 1.0*/
